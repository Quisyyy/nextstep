...
function populateFormWithData(data) {
  ...
  const addressParts = [];
  if (data.streetDetails) addressParts.push(data.streetDetails);
  if (data.barangay) addressParts.push(data.barangay);
  if (data.city) addressParts.push(data.city);
  if (data.province) addressParts.push(data.province);
  document.getElementById("address").value = addressParts.length
    ? addressParts.join(", ")
    : "";

  // Birthday
  if (data.birth_month)
    document.getElementById("birth-month").value = data.birth_month;
  if (data.birth_day)
    document.getElementById("birth-day").value = data.birth_day;
  if (data.birth_year)
  ...
  if (data.graduated_year)
    document.getElementById("graduated").value = data.graduated_year;
  ...
  if (document.getElementById("openForMentorship")) {
    document.getElementById("openForMentorship").checked =
      data.open_for_mentorship || false;
  }
...
async function flushSubmitQueue() {
  try {
    ...
    const ready = await ensureSupabaseReady(5000);
    if (!ready) {
      console.info("Supabase not ready; deferring queue flush");
      return;
    }
    ...
    if (!error) {
      ...
      try {
        window.dispatchEvent(
          new CustomEvent("alumni:flushed", {
    ...
    console.warn("flushSubmitQueue failed", e);
  }
}
...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  if (!saveButton) {
    ...
    if (window.debugWidget)
      window.debugWidget.log("❌ Save button #saveProfile not found in DOM");
    return;
  }

  // Decide mode: explicit edit via URL OR implicit load by logged-in email
  const editProfileId = getEditProfileId();
  const loggedEmail = (localStorage.getItem("currentUserEmail") || "")
    .trim()
    .toLowerCase();
  ...
  if (editProfileId) {
    ...
    if (existingData) {
      ...
      if (status)
        status.textContent = "Warning: Could not load existing profile data";
    }
  } else if (loggedEmail) {
  ...
  saveButton.addEventListener("click", async function (e) {
    ...
    if (window.debugWidget)
      window.debugWidget.log(
        `📝 Payload prepared: ${Object.keys(payload).filter((k) => payload[k]).length} fields`,
      );

    try {
      if (window.debugWidget)
        window.debugWidget.log("⏳ Checking Supabase readiness...");
      ...
      if (ready) {
        ...
        if (isEditMode) {
          ...
          if (window.debugWidget)
            window.debugWidget.log("✅ Supabase ready, attempting update");
          ...
          if (window.debugWidget)
            window.debugWidget.log(
              `📊 Update result: ${error ? "ERROR" : "SUCCESS"}`,
            );
        } else {
          // INSERT new record
          if (window.debugWidget)
            window.debugWidget.log("✅ Supabase ready, attempting insert");
          const result = await window.supabase
            .from("alumni_profiles")
            .insert([payload])
            .select();
          ...
          if (window.debugWidget)
            window.debugWidget.log(
              `📊 Insert result: ${error ? "ERROR" : "SUCCESS"}`,
            );
        }
        ...
        if (window.debugWidget)
          window.debugWidget.log(
            `✅ Successfully ${isEditMode ? "updated" : "saved"} to Supabase`,
          );
        ...
        if (window.debugWidget)
          window.debugWidget.log("❌ Supabase not ready after timeout");
      }
    } catch (err) {
      console.warn("Supabase insert failed", err);
      if (window.debugWidget)
        window.debugWidget.log(`❌ Supabase error: ${err.message || err}`);
    ...
    try {
      if (window.debugWidget)
        window.debugWidget.log("💾 Saving to localStorage queue");
      ...
      if (window.debugWidget)
        window.debugWidget.log(
          `💾 Queued locally (${queue.length} items total)`,
        );
    } catch (e) {
      console.error("local save failed", e);
      if (window.debugWidget)
        window.debugWidget.log(`❌ Local save failed: ${e.message}`);
