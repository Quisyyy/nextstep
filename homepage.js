...
(async function () {
  ...
  const fallback = {
    ...
    about: {
      ...
      mission:
        "Next Step is committed to fostering lifelong connections between graduates and their alma mater by providing a dynamic platform for alumni engagement, career tracking, and professional growth.",
      vision:
        "To be the leading digital hub that empowers alumni to thrive, inspire future generations, and shape institutional excellence through collaboration, innovation, and shared success.",
    },
  };
  ...
  function apply(data) {
    try {
      ...
      if (hTitle && data.hero && data.hero.title)
        hTitle.textContent = data.hero.title;
      if (hText && data.hero && data.hero.text)
        hText.textContent = data.hero.text;
      if (hCta && data.hero) {
        hCta.textContent = data.hero.ctaText || hCta.textContent;
        if (data.hero.ctaHref) hCta.setAttribute("href", data.hero.ctaHref);
      }

      if (aHeading && data.about && data.about.heading)
        aHeading.textContent = data.about.heading;
      if (aLead && data.about && data.about.lead)
        aLead.textContent = data.about.lead;
      ...
      if (mission && data.about && data.about.mission)
        mission.textContent = data.about.mission;
      if (vision && data.about && data.about.vision)
        vision.textContent = data.about.vision;
    } catch (e) {
...
document.addEventListener("DOMContentLoaded", async () => {
  const email = window.updateAuthNav
