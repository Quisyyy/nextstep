  ...
  try {
    ...
    if (!error) {
      ...
      if (window.debugWidget)
        window.debugWidget.log("✅ Database tables verified");
      return true;
    }
    ...
    if (window.debugWidget)
      window.debugWidget.log(`⚠️ Database issue: ${error.message}`);

    // Show user-friendly message
    const statusElements = document.querySelectorAll(
      "#saveStatus, #signupStatus",
    );
    ...
    if (window.debugWidget)
      window.debugWidget.log(`❌ Database check error: ${e.message}`);
    return false;
  }
}
  ...
  if (attempts >= maxAttempts) {
    ...
    if (window.debugWidget)
      window.debugWidget.log(
        "⚠️ Supabase client timeout during database setup",
      );
  }
})();
